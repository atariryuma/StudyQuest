...
```
   ...
   `scripts/check-deps.js` により `npm test` 実行前に依存ライブラリの存在が確認さ
   れます。必ず上記コマンドでインストールを済ませてからテストを実行してください。
